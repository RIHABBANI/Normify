...
export const NormsChaptersTable = () => {
    const [normChapters, setNormChapters] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const fetchNormChapters = async () => {
            try {
                const data = await getNormChapters();
                console.log(data);
                setNormChapters(data);
            } catch (err) {
                console.error('Error fetching norm chapters:', err.message);
            } finally {
                setLoading(false);
            }
        };
            ...
            <table className="w-full text-xs text-left text-gray-900 bg-white">
                <thead className="text-xs uppercase text-gray bg-white-700">
                    <tr>
                        ...
                        <th scope="col" className="px-4 py-2">Chapter Reference</th>
                        <th scope="col" className="px-4 py-2">Chapter Title</th>
                        <th scope="col" className="px-4 py-2">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    {normChapters.map((normChapter, index) => (
                        <tr
                            key={index}
                            className={`${
                        ...
                        >
                            ...
                            <td className="px-4 py-2">{normChapter.chapter_ref}</td>
                            <td className="px-4 py-2">{normChapter.chapter_title}</td>
...
export default NormsChaptersTable;